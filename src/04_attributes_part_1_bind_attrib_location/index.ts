...
export default async function _004_attributes_part_1_bind_Attrib_location(context: WebGL2RenderingContext) {
    const aPositionLocation = 0;
    const aPointSizeLocation = 1;
    const aColorLocation = 2;
    const glShaderInterface = await configureShaders(context, './04_attributes_part_1_bind_attrib_location', 'vSource', 'fSource', true, { aPositionLocation, aPointSizeLocation, aColorLocation });
    //^----Attributes----^\\
    /**
